...
def _read_csv(dataset_id: str):
    ...
    Returns (cpu_list, ram_list, task_list) or raises HTTPException 404.
    """
    csv_path = os.path.join(_DATA_DIR, f"{dataset_id}_5min.csv")
    if not os.path.isfile(csv_path):
        raise HTTPException(status_code=404, detail=f"Dataset '{dataset_id}' not found on disk.")
    ...
    if not cpu_vals:
        raise HTTPException(status_code=500, detail=f"Dataset '{dataset_id}' CSV is empty or malformed.")
...
def get_dataset_series(dataset_id: str, length: int = 100):
    """
    Return the real cpu_utilization time series from the processed CSV.
    Optional `length` parameter slices the first N rows (default 100).
    """
